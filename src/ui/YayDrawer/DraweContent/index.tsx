...
const DrawerContent: FunctionComponent<DrawerContentProps> = ({
  ...
  return (
    <>
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 bg-black/40 z-50 outline-none" />
        <Drawer.Content className="bg-[#b7b7b7] flex flex-col rounded-t-[10px] h-full w-[500px] mt-24 fixed bottom-0 right-0 z-50 outline-none">
          <span className="text-black mx-2">{goal.parentTaskId ? "indietro" : null}</span>
          <div className="p-4 bg-[#b7b7b7] flex-1 h-full outline-none">
            <div className="max-w-lg mx-auto outline-none flex flex-col justify-start items-center">
              ...
              <Drawer.Title className="font-medium text-3xl mb-2 mt-2 text-black outline-none w-full">
                <Input
                  radius="none"
              ...
              <span className="w-full text-black text-start">
                Add a description{" "}
                <span className="bg-blue rounded-full relative w-3 h-3">+</span>
              </span>
              ...
              <div className="w-full mt-5">
                <Accordion className="!px-0 ">
                  <AccordionItem
                    ...
                    classNames={{
                      indicator: "mx-2 p-1 rounded-full bg-black text-white",
                      titleWrapper: "px-0",
                        ...
                        {goal.subTasks.map((g: Goal) => {
                          return (
                            <div key={g?.id} className="flex items-center">
                              ...
                              <div className="pr-2">
                                <Chip size="sm" className="rounded bg-red-600">
                                  delete
                                </Chip>
                              </div>
                            </div>
